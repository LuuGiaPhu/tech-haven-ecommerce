...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateCuteProgress() {
        ...
        if (progress >= 100) {
            ...
            const animationDelay = isMobile ? 150 : 300;
            const scaleTransform = isMobile ? 'scale(1.05)' : 'scale(1.1)';
            
            setTimeout(() => {
                if (!isMobile) {
                    loadingScreen.style.transform = scaleTransform;
                }
                setTimeout(() => {
    ...
    const fallbackTimeout = isMobile ? 3000 : 4000;
    setTimeout(() => {
...
window.handleLogout = function() {
    ...
    // Đã được xóa - sử dụng signOutUser từ index.ejs
};

...
let functions, onRequest, onSchedule, onDocumentUpdated;
if (!isLocal) {
...
app.get('/product/:id', async (req, res) => {
  ...
  try {
    ...
    let relatedProducts = [];
    try {
      console.log('🔍 Looking for related products in category:', product.category);
      
      // First try: same category
      let relatedQuery = await db.collection('products')
        .where('category', '==', product.category)
        .limit(20)
        .get();
      
      console.log('📊 Initial query returned:', relatedQuery.docs.length, 'products');
      
      relatedProducts = relatedQuery.docs
        .filter(doc => {
          const data = doc.data();
          const isNotCurrent = doc.id !== productId;
          const isNotVariant = !data.parent_id_product;
          
          return isNotCurrent && isNotVariant;
        })
      ...
      console.log('✅ After filtering same category:', relatedProducts.length, 'products');
      
      // If not enough related products, get more from other categories
      if (relatedProducts.length < 4) {
        console.log('⚠️ Not enough related products, fetching from other categories');
        const moreQuery = await db.collection('products')
          .limit(30)
          .get();
        
        const moreProducts = moreQuery.docs
          .filter(doc => {
            const data = doc.data();
            const isNotCurrent = doc.id !== productId;
            const isNotVariant = !data.parent_id_product;
            const notAlreadyIncluded = !relatedProducts.find(p => p.id === doc.id);
            
            return isNotCurrent && isNotVariant && notAlreadyIncluded;
          })
        ...
        console.log('✅ After adding from other categories:', relatedProducts.length, 'products');
      }
...
// GET /api/users/:userId/coin - Get user's coin balance
app.get('/api/users/:userId/coin', async (req, res) => {
  try {
    const { userId } = req.params;
    ...
    const userData = userDoc.data();
    const coinBalance = userData.coin || 0;

    console.log(`✅ Coin balance: ${coinBalance.toLocaleString('vi-VN')} coin`);

    res.json({
      success: true,
      coin: coinBalance,
      coinValue: coinBalance // 1 coin = 1 VND
    });
...
// GET /api/users/email/:email/coin - Get user's coin balance by email (for guest checkout)
app.get('/api/users/email/:email/coin', async (req, res) => {
  try {
    const email = decodeURIComponent(req.params.email);
    console.log('💰 Getting coin balance for email:', email);
    ...
    console.log(`✅ Found user ${userId} with coin balance: ${coinBalance.toLocaleString('vi-VN')} coin`);

    res.json({
      success: true,
      coin: coinBalance,
      coinValue: coinBalance, // 1 coin = 1 VND
      userId: userId // Return userId so frontend can use it for checkout
    });

  } catch (error) {
    console.error('❌ Error getting coin balance by email:', error);
    res.status(500).json({
      success: false,
      error: 'Failed to get coin balance'
    });
...
// POST /api/users/:userId/coin/use - Use coin for payment
app.post('/api/users/:userId/coin/use', async (req, res) => {
  try {
    const { userId } = req.params;
    const { amount, orderId, orderTotal } = req.body;
    
    console.log(`💸 Using ${amount} coin for user: ${userId}, order: ${orderId}`);
    ...
    if (currentCoin < amount) {
      return res.status(400).json({
        success: false,
        error: `Không đủ coin. Số dư hiện tại: ${currentCoin.toLocaleString('vi-VN')} coin`,
        currentCoin: currentCoin
      });
    }

    // Check if coin amount exceeds order total
    if (amount > orderTotal) {
      return res.status(400).json({
        success: false,
        error: `Số coin sử dụng không thể vượt quá tổng đơn hàng (${orderTotal.toLocaleString('vi-VN')} VNĐ)`,
        maxCoinUsable: orderTotal
      });
    }
    ...
    await db.collection('coin_transactions').add({
      ...
      balanceBefore: currentCoin,
      balanceAfter: newCoinBalance,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      description: `Sử dụng coin thanh toán đơn hàng #${orderId}`
    });

    console.log(`✅ Coin used: -${amount.toLocaleString('vi-VN')} coin (Remaining: ${newCoinBalance.toLocaleString('vi-VN')} coin)`);

    res.json({
      success: true,
      message: 'Đã sử dụng coin thành công',
      coinUsed: amount,
      newCoinBalance: newCoinBalance,
      discountAmount: amount // 1 coin = 1 VND discount
    });
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to use coin: ' + error.message
    });
  }
});

// GET /api/users/:userId/coin/transactions - Get coin transaction history
app.get('/api/users/:userId/coin/transactions', async (req, res) => {
  try {
    const { userId } = req.params;
    const { limit = 50 } = req.query;
    
    console.log('📜 Getting coin transaction history for user:', userId);
    ...
    console.log(`✅ Found ${transactions.length} coin transactions`);
    ...
    res.status(500).json({
      success: false,
      error: 'Failed to get coin transactions'
    });
  }
});

// =====================================
// END COIN MANAGEMENT API ENDPOINTS
// =====================================
...
app.post('/api/bills', async (req, res) => {
  try {
    const { 
      ...
      products, 
      totalAmount,
      coinUsed, // Add coin usage parameter
      couponUserId, 
    ...
    // Deduct coin if used
    if (coinUsed && coinUsed > 0 && userId && userId !== 'guest' && !userId.startsWith('guest_')) {
      console.log(`💰 Deducting ${coinUsed.toLocaleString('vi-VN')} coin from user ${userId}...`);
      ...
      try {
        ...
        if (coinUsed > totalAmount) {
          return res.status(400).json({
            success: false,
            error: `Số coin sử dụng không thể vượt quá tổng đơn hàng (${totalAmount.toLocaleString('vi-VN')} VNĐ)`,
            maxCoinUsable: totalAmount
          });
        }
        
        // Deduct coin from user balance
        const newCoinBalance = currentCoin - coinUsed;
        
        await db.collection('users').doc(userId).update({
          coin: newCoinBalance
        });
        
        // Create coin transaction record (will update orderId later)
        const tempBillId = `temp_${Date.now()}`;
        await db.collection('coin_transactions').add({
          userId: userId,
          type: 'spend',
          amount: coinUsed,
          orderId: tempBillId, // Temporary ID, will be updated after bill creation
          balanceBefore: currentCoin,
          balanceAfter: newCoinBalance,
        ...
        console.log(`✅ Coin deducted: ${coinUsed.toLocaleString('vi-VN')} coin. New balance: ${newCoinBalance.toLocaleString('vi-VN')} coin`);
        
      } catch (error) {
        console.error('❌ Error deducting coin:', error);
        return res.status(500).json({
          success: false,
          error: 'Lỗi khi trừ coin: ' + error.message
        });
      }
    }
    ...
    const billData = {
      ...
      coinUsed: coinUsed || 0, // Save coin used in bill
...
app.post('/api/bills/:billId/confirm-delivery', async (req, res) => {
  try {
    ...
    // Auto-update membership points (Coin will be handled by Firebase Trigger)
    let tierPromoted = false;
    let membershipData = null;
    let oldLevel = null;
    let coinWillBeEarned = 0;
    let userDoc = null;
    ...
    try {
      ...
      // Calculate coin that WILL be earned by trigger (10% of order value)
      const orderAmount = billData.totalAmount || 0;
      coinWillBeEarned = Math.floor(orderAmount * 0.10);

      // Update ONLY membership, NOT coin (trigger will handle coin)
      await db.collection('users').doc(userId).update({
        points: points,
        membershipLevel: membership.level,
        lastMembershipUpdate: admin.firestore.FieldValue.serverTimestamp()
      ...
      console.log(`💰 Coin will be earned by trigger: +${coinWillBeEarned.toLocaleString('vi-VN')} coin`);
    ...
    res.json({
      success: true,
      message: 'Đã xác nhận nhận hàng thành công! Hệ thống sẽ tự động cập nhật coin.',
      billId: billId,
      tierPromoted: tierPromoted,
      membership: membershipData,
      coinWillBeEarned: coinWillBeEarned,
      note: 'Coin sẽ được cộng tự động bởi hệ thống trong vài giây'
    });
...
app.post('/api/bills/:billId/return', async (req, res) => {
  try {
    ...
    // Note: Coin deduction is handled by Firebase Trigger (onBillReturned)
    // to avoid duplicate deduction. Trigger will auto-run when status changes to 'returned'
    const orderAmount = billData.totalAmount || 0;
    const coinEarnedFromOrder = Math.floor(orderAmount * 0.10);
    
    console.log(`ℹ️ Coin deduction (${coinEarnedFromOrder.toLocaleString('vi-VN')} coins) will be handled by Firebase Trigger automatically`);
    ...
    console.log(`⏳ Waiting for Firebase Trigger to deduct ${coinEarnedFromOrder.toLocaleString('vi-VN')} coins...`);
    
    res.json({
      success: true,
      message: coinEarnedFromOrder > 0 
        ? `Đã xử lý trả hàng thành công! Số lượng sản phẩm đã được hoàn về kho. Hệ thống sẽ tự động trừ ${coinEarnedFromOrder.toLocaleString('vi-VN')} coin (10% giá trị đơn hàng ${orderAmount.toLocaleString('vi-VN')} VND).`
        : 'Đã xử lý trả hàng thành công! Số lượng sản phẩm đã được hoàn về kho. Thông tin hoàn tiền đã được lưu.',
      billId: billId,
      coinWillBeDeducted: coinEarnedFromOrder,
...
if (isLocal) {
  ...
  // Firestore Trigger: Auto-update coin when bill status changes to "completed"
  exports.onBillCompleted = onDocumentUpdated('bills/{billId}', async (event) => {
    ...
    if (beforeData.status !== 'completed' && afterData.status === 'completed') {
      console.log(`🎉 Bill ${billId} completed! Auto-updating coin...`);
      
      const userId = afterData.userId;
      const orderAmount = afterData.totalAmount || 0;
      
      if (!userId || orderAmount <= 0) {
        console.log('⚠️ Invalid userId or orderAmount, skipping coin update');
        return null;
      }
      
      try {
        // Delete any old "deduct_return" transaction for this bill (if user is re-testing)
        const oldDeductTransactions = await admin.firestore()
          .collection('coin_transactions')
          .where('orderId', '==', billId)
          .where('type', '==', 'deduct_return')
          .get();
        
        if (!oldDeductTransactions.empty) {
          console.log(`🗑️ Deleting ${oldDeductTransactions.size} old deduct_return transaction(s) for bill ${billId}`);
          const batch = admin.firestore().batch();
          oldDeductTransactions.docs.forEach(doc => batch.delete(doc.ref));
          await batch.commit();
        }
        
        // Check if coin has already been earned for this bill
        const existingEarnTransaction = await admin.firestore()
          .collection('coin_transactions')
          .where('orderId', '==', billId)
          .where('type', '==', 'earn')
          .limit(1)
        ...
        if (!existingEarnTransaction.empty) {
          console.log(`⚠️ Coin already earned for bill ${billId}, skipping to prevent double earning`);
          return { success: false, message: 'Already earned' };
        }
        
        // Get current coin balance
        const userDoc = await admin.firestore().collection('users').doc(userId).get();
        const currentCoin = userDoc.exists ? (userDoc.data().coin || 0) : 0;
        
        // Calculate coin earned (10% of order value)
        const coinEarned = Math.floor(orderAmount * 0.10);
        const newCoinBalance = currentCoin + coinEarned;
        ...
        // Log transaction
        await admin.firestore().collection('coin_transactions').add({
          userId: userId,
          type: 'earn',
          amount: coinEarned,
          orderId: billId,
          ...
          balanceAfter: newCoinBalance,
          timestamp: admin.firestore.FieldValue.serverTimestamp(),
          description: `Earned from completed order #${billId}`
        });
        
        console.log(`💰 Coin updated for user ${userId}: ${currentCoin.toLocaleString('vi-VN')} → ${newCoinBalance.toLocaleString('vi-VN')} (+${coinEarned.toLocaleString('vi-VN')} coin)`);
        
        return { success: true, coinEarned, newBalance: newCoinBalance };
      } catch (error) {
  ...
  // Firestore Trigger: Auto-deduct coin when bill status changes to "returned"
  exports.onBillReturned = onDocumentUpdated('bills/{billId}', async (event) => {
    const billId = event.params.billId;
    const beforeData = event.data.before.data();
    const afterData = event.data.after.data();
    
    // Only proceed if status changed to "returned" (not already returned)
    if (beforeData.status !== 'returned' && afterData.status === 'returned') {
      console.log(`↩️ Bill ${billId} returned! Auto-deducting coin...`);
      
      const userId = afterData.userId;
      const orderAmount = afterData.totalAmount || 0;
      
      if (!userId) {
        console.log('⚠️ Invalid userId, skipping coin deduction');
        return null;
      }
      
      // Calculate coin EARNED when order was completed (10% of order value)
      const coinEarnedFromOrder = Math.floor(orderAmount * 0.10);
      
      if (coinEarnedFromOrder <= 0) {
        console.log(`ℹ️ No coins were earned from this order (order amount: ${orderAmount.toLocaleString('vi-VN')} VND), skipping coin deduction`);
        return null;
      }
      
      console.log(`💰 This order earned ${coinEarnedFromOrder.toLocaleString('vi-VN')} coins when completed (10% of ${orderAmount.toLocaleString('vi-VN')} VND)`);
      
      try {
        let alreadyReversed = false;
        let coinBeforeDeduction = 0;
        
        // Delete any old "earn" transaction for this bill AND reverse the coin addition
        const oldEarnTransactions = await admin.firestore()
          .collection('coin_transactions')
          .where('orderId', '==', billId)
          .where('type', '==', 'earn')
          .get();
        
        if (!oldEarnTransactions.empty) {
          console.log(`🗑️ Found ${oldEarnTransactions.size} old earn transaction(s) for bill ${billId} - will reverse`);
          
          // Get the coin amount that was earned
          const earnedAmount = oldEarnTransactions.docs[0].data().amount || 0;
          
          // Get current coin and reverse the earning
          const userDoc = await admin.firestore().collection('users').doc(userId).get();
          const currentCoin = userDoc.exists ? (userDoc.data().coin || 0) : 0;
          const coinAfterReversing = currentCoin - earnedAmount;
          
          console.log(`↩️ Reversing earn: ${currentCoin.toLocaleString('vi-VN')} - ${earnedAmount.toLocaleString('vi-VN')} = ${coinAfterReversing.toLocaleString('vi-VN')}`);
          
          // Update user coin (reverse the earning)
          await admin.firestore().collection('users').doc(userId).update({
            coin: coinAfterReversing
          });
          
          // Delete old earn transactions
          const batch = admin.firestore().batch();
          oldEarnTransactions.docs.forEach(doc => batch.delete(doc.ref));
          await batch.commit();
          
          console.log(`✅ Old earn transaction deleted and coin reversed to ${coinAfterReversing.toLocaleString('vi-VN')}`);
          
          alreadyReversed = true;
          coinBeforeDeduction = currentCoin;
        }
        
        // Check if coin has already been deducted for this bill
        const existingDeductTransaction = await admin.firestore()
          .collection('coin_transactions')
          .where('orderId', '==', billId)
          .where('type', '==', 'deduct_return')
          .limit(1)
        ...
        if (!existingDeductTransaction.empty) {
          console.log(`⚠️ Coin already deducted for bill ${billId}, skipping to prevent double deduction`);
          return { success: false, message: 'Already deducted' };
        }
        
        // If we already reversed the earning, just log the transaction (don't deduct again)
        if (alreadyReversed) {
          const finalCoin = coinBeforeDeduction - coinEarnedFromOrder;
          
          // Log transaction for audit trail
          await admin.firestore().collection('coin_transactions').add({
            userId: userId,
            type: 'deduct_return',
            amount: coinEarnedFromOrder,
            orderId: billId,
            balanceBefore: coinBeforeDeduction,
            balanceAfter: finalCoin,
            timestamp: admin.firestore.FieldValue.serverTimestamp(),
            description: `Reversed earning from returned order #${billId} (10% of ${orderAmount.toLocaleString('vi-VN')} VND)`
          });
          
          console.log(`✅ Return transaction logged. Final balance: ${finalCoin.toLocaleString('vi-VN')} coin`);
          return { success: true, coinDeducted: coinEarnedFromOrder, newBalance: finalCoin };
        }
        
        // If no old earn transaction found, deduct coin normally
        const userDoc = await admin.firestore().collection('users').doc(userId).get();
        const currentCoin = userDoc.exists ? (userDoc.data().coin || 0) : 0;
        const newCoinBalance = currentCoin - coinEarnedFromOrder;
        ...
        await admin.firestore().collection('coin_transactions').add({
          ...
          balanceAfter: newCoinBalance,
          timestamp: admin.firestore.FieldValue.serverTimestamp(),
          description: `Deducted ${coinEarnedFromOrder.toLocaleString('vi-VN')} coins earned from returned order #${billId} (10% of ${orderAmount.toLocaleString('vi-VN')} VND)`
        });
        
        console.log(`💸 Coin deducted for user ${userId}: ${currentCoin.toLocaleString('vi-VN')} → ${newCoinBalance.toLocaleString('vi-VN')} (-${coinEarnedFromOrder.toLocaleString('vi-VN')} coin)`);
        
        return { success: true, coinDeducted: coinEarnedFromOrder, newBalance: newCoinBalance };
      } catch (error) {
        console.error('❌ Error deducting coin:', error);
        return null;
      }
